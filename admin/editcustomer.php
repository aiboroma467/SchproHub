...
$sql="update neworder set FirstName=:fname,LastName=:lname,Department=:department,Phonenumber=:mobileno where id=:eid";
...
<?php 
$eid=intval($_GET['empid']);
$sql = "SELECT * from  neworder where id=:eid";
$query = $dbh -> prepare($sql);
$query -> bindParam(':eid',$eid, PDO::PARAM_STR);
$query->execute();
...
<?php $sql = "SELECT Department from neworder";
$query = $dbh -> prepare($sql);
$query->execute();
$results=$query->fetchAll(PDO::FETCH_OBJ);
$cnt=1;
...
{   ?>                                            
<option value="<?php echo htmlentities($resultt->Department);?>"><?php echo htmlentities($resultt->Department);?></option>
<?php }} ?>
</select>
</div>
<!--
